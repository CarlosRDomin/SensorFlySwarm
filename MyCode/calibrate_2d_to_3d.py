...
from full_control_with_cam import Spotter, FakeWorkerDrone
...
def estimate_3d_location(radius_in_m, F):
	...
	while cv2.waitKey(1) < 0:
		...
		if cf_curr_pos is not None:
			...
			cv2.circle(cv.cv_cam_frame, tuple(cf_curr_pos[0:2].astype(int)), int(cf_curr_pos[2]), (255, 0, 0), 1)
			cv2.putText(cv.cv_cam_frame, "x={p[0]:.2f}m, y={p[1]:.2f}m, z={p[2]:.2f}m".format(p=world_coords), (50, cv.cv_cam_frame.shape[0]-50), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 0, 200), 1, cv2.LINE_AA)
		else:
...
# 		if cf_curr_pos is not None:
# 			radius_in_px = cf_curr_pos[2]
# 			cf_curr_pos_in_m = [-radius_in_m*F/radius_in_px,
# 						   (cf_curr_pos[0] - cv.cv_cam_frame.shape[1]/2)*radius_in_m/radius_in_px,
# 						   (cv.cv_cam_frame.shape[0]/2 - cf_curr_pos[1])*radius_in_m/radius_in_px]
# 			cv2.circle(cv.cv_cam_frame, tuple(cf_curr_pos[0:2].astype(int)), int(cf_curr_pos[2]), (255, 0, 0), 1)
# 			cv2.putText(cv.cv_cam_frame, "x={p[0]:.2f}m, y={p[1]:.2f}m, z={p[2]:.2f}m".format(p=cf_curr_pos_in_m), (50, cv.cv_cam_frame.shape[0]-50), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 0, 200), 1, cv2.LINE_AA)
# 		else:
# 			cv2.putText(cv.cv_cam_frame, "CAN'T TRACK CF!!", (50, cv.cv_cam_frame.shape[0]-50), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 255), 1, cv2.LINE_AA)
# 		cv2.imshow(win_title, cv.cv_cam_frame)
#
...
if __name__ == '__main__':
	calibrate_cam = False
	F = find_focal_length_in_px(0.02, 1.00) if calibrate_cam else 1250.0
	estimate_3d_location(0.02, F)
